%% Image Read
I = im2double(imread('C:\Users\hjjiang\Desktop\大三上\数字图像处理\作业\综合作业1\latent.bmp'));
...
coordinate = zeros(ynum(2), xnum(2));
for i = 1:xnum(2)
    for j = 1:ynum(2)
        block_32{j,i} = I(ycenter(j)-block_half_size+1:ycenter(j)+block_half_size,xcenter(i)-block_half_size+1:xcenter(i)+block_half_size);  % 第一个维度是高
        ...
        temp_amplitude = block_32_DFT_amplitude{j,i};
        max_amplitude = max(max(temp_amplitude));
        [y,x] = find(temp_amplitude == max_amplitude);
        temp_amplitude(y,x) = 0;
        second_max_amplitude = max(max(temp_amplitude));
        ratio = second_max_amplitude/max_amplitude;
        if (0.15 < ratio) && (ratio < 0.65) && (max_amplitude < 500)
            coordinate(j,i) = 1;
        end
    end
end
...
D0=10;
H = glpf(D0,P);
F = fftshift(fft2(coordinate,P,P));
G = F.*H;
...
%% Show Result
smooth_block_frequency_image = zeros(768,800);
block_frequency_image = zeros(768,800);
fingerprint_area = zeros(768,800);
for i = 1:xnum(2)
    for j = 1:ynum(2)
        if coordinate(j,i) == 1
            temp = zeros(8);
...
subplot(2,4,2),imshow(fingerprint_area,[]),title('Spatial Enhanced Image')
